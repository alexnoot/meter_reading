...
def aidon_callback(fields):
    ...
    if 'p_act_in' in fields:
        hi.post("aidon", "power", "%.03f" % (fields['p_act_in'] / 1000.0), hass_name="Power", hass_unit="kW", ts=ts)
    ...
    if 'energy_act_in' in fields:
        hi.post("aidon", "energy", "%.02f" % fields['energy_act_in'], hass_name="Energy", hass_unit="kWh", ts=ts)
